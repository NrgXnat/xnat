...
| 1-11 | Remove Restlet `ext.fileupload` (dropped after 2.5.2) → commons-fileupload2 jakarta-servlet6 | 🟢 | **Superseded 2026-07-22 (commit `bece09806`):** the transitional XNAT-owned `RestletFileUpload`/`RepresentationContext` bridge is deleted; upload path now uses `JakartaServletFileUpload<DiskFileItem,…>` parsing the jakarta `HttpServletRequest` directly. See the fileupload2 note below |
...
`SecureResource` okParity commits (`5c6f81467` + `05ff1c142`) — the parallel turbine7-clean effort hit
...
second migration (`features/turbine7-clean` `f13a02162` — that branch has since been deleted from local
and origin, so this SHA no longer resolves anywhere; kept for the record, it was never a commit on this
branch), adapted per-hunk to this branch. Commit
`bece09806`. **GA-risk tradeoff accepted knowingly**: fu2's latest is the `2.0.0-M5` milestone and it
...
on a stray `%` (e.g. a `LIKE '%x%'` wildcard) and turned `+`→space. Commit `9dc73d24e`. Verified: t1008.1
...
1. *Defensive, core:* rebind `$user` in the custom-screen macros + `NoMenuTop`/
   `NoninteractiveTop` (commit `7c1f5d7eb`) so the nav survives *any* template that nulls
   `$user`.
...
+ verified: core compiles, plugin jars package the fixed templates dead-attr-free. Commits: core `03eef655f`,
...
empirically (userHelper pages render with 0 NPEs). Commits: core `33be4a712`, xnat_cr_plugin `8975073`; rebuilt +
...
- *(3) jackson-datatype-hibernate5 javax variant — same class as core `6a28fe3ce`, here on container-service's own
...
three (core commit `c333e9807`). List-receiver `.contains($x)` sites (`$read_projects`/`$exclusions`/`$completed`…)
...
commits onto `origin/develop` (`a2559a027`); backup at `backup/jakarta-cutover-1b2cbab2b`. **One textual conflict:**
...
compile here.** Prior instance: `Representation.createEmpty()` (`6b61ed331`). This time XNAT‑8784's audit feature
...
Fixed in `1524f5667`. **Audit.** Swept every line develop *added* across all 85 changed files (diff hunks, not whole
...
**1-34 🟢 — Phase-A cutover triage; 1-24 root-caused (form-encoded PUT loses its body) and closed;
1-23 decided** (2026-08-19). Triaged every non-green item ahead of opening cutover PRs. Dispositions:
**1-2** SS6 → close green (landed + runtime-verified; residuals are 1-23 plus SS7-prep deprecation
warnings — note SS6 deliberately **keeps the deprecated `authorizeRequests`**, because
`authorizeHttpRequests` builds an `AuthorizationFilter` that `UpdateSecurityFilterHandlerMethod` never
sees → login redirect loop; `AuthorizationManager` port is an SS7 task, and this belongs in the release
notes as intentional debt). **1-13** CI flip → scheduled at merge, *not* a review gate; note the change
lives in the separate `NrgXnat/xnat-ci-workflows` repo and CI must run **JDK 21** or
`verifyNoOrphanedJavaxEE` (wired into `check`) dies on the aspectjweaver zip64 CEN issue (1-33).
**1-17** → decoupled from the release entirely (tracker already states XNAT does not depend on it).
**1-19**, **1-22** → defer to 1.12, but both owe **release-note** content (1-19's `getBodyVariable`
behavior change; 1-22's deliberately-shipped javax jars + why they're allowlisted). **1-23** → decided,
see the row. **1-24** → fixed, below.

**1-24 real root cause (the parked hypothesis was wrong).** The tracker recorded a "`/services/auth`
route-scoring conflict on `rootRouter`". It is not: the route resolves correctly. `XnatSecureGuard` has
**no URI awareness at all** — it authenticates whatever reaches it and can only answer 401, never 404 —
so a 404 proved the guard never handled the request, i.e. routing had already succeeded.
**Mechanism:** Restlet 2.6's servlet connector builds the entity for `application/x-www-form-urlencoded`
from the servlet parameter map; per Servlet 6.0 §3.1 the container populates that map for **POST only**,
so a form-encoded **PUT** yields an empty entity and `isEntityAvailable() == false` even though the client
sent a body. `AuthenticationRestlet`'s constructor then threw a bare `java.lang.Exception`, and
`XnatServerResourceFinder` maps any non-`ResourceException` to `null`, which Restlet renders as **404**.
Restlet 1.1 always exposed the raw body, hence the regression. **Why it hid for weeks:** the actual
diagnostic (`Unable to instantiate resource … AuthenticationRestlet / java.lang.Exception: You must
provide authentication credentials…`) is written by the Finder's JUL logger into Tomcat's
`localhost.<date>.log` — **not** `catalina.out`, which is the only file anyone was grepping.
**A/B proof (dave-xl, pre-fix):** `PUT`+form → **404**; `PUT`+`text/plain` → 401; `POST`+form → 401 —
exactly the POST-only parsing rule. **Fix (shared altitude):** new
`SecureResource#getRequestBodyText()` reads the body from the Restlet entity → else the raw servlet
reader (intact on PUT, never parsed by the container) → else the parameter map re-encoded (POST, stream
already drained); `AuthenticationRestlet` uses it and throws `ResourceException` so a missing body is
**400** and a GET is **405** instead of a misleading 404. **Audit (bug class = "resource parses the raw
request entity itself"): 4 sites.** `AuthenticationRestlet` fixed; **`IpWhitelist` fixed** (same defect
twice — a constructor `PUT && !isEntityAvailable()` guard that rejected valid form PUTs with 412, plus a
`handlePut()` entity read); **`UserSettingsRestlet` fixed** (handles PUT, read the entity in its
constructor → form PUT payloads silently emptied); `ItemValuesRepresentation` **left as-is** (guards on
`isEntityAvailable() && size > 0` and falls back to a default rather than failing). **Result: verified on
dave-xl** — PUT+form/PUT+text/POST+form all 401, no-body 400, GET 405, and `security.log` logs
`Searched for user auth record by username 'nosuchuser'` once per credential-bearing request, proving the
credentials are parsed and reach the auth provider on the previously-broken path. Commit `97695ba97`.
**Follow-up worth considering:** `XnatServerResourceFinder` rewriting *any* non-`ResourceException`
...
2. **A squash orphans this document.** The tracker cites **21 commit SHAs** as evidence across its    items. Squashing invalidates that provenance, and this file is the institutional memory of the migration.
   *(Corrected 2026-08-27 — this originally read "all sampled ones currently resolve", which was measured
   with `git cat-file -e`. That succeeds for **dangling** objects, so it proved far less than it implied:
   16 of the 22 were in fact already unreachable, orphaned by earlier rebases. See 1-40.)*
...
and invites conflicts. And the blanket form is unsafe — **`5d6861db0` is titled `docs:` but also carries
...
**1-40 🟢 — final rebase onto `develop`, and the tracker's SHA citations re-pointed; branch switches to
merge-only from here** (2026-08-27).

**Why this was the *final* rebase.** Investigating "should we keep rebasing or start merging" surfaced that
**16 of the 22 SHAs this document cites as evidence were already unreachable** — orphaned by the earlier
rebases (2026-08-06, 08-17, 08-24). They resolved on the migration machine only as dangling objects awaiting
`git gc`; in a fresh clone, or for any reviewer, they did not exist. The provenance argument used in 1-39 to
oppose squashing turned out to apply to rebasing too, just more slowly. **Method note:** the original check
used `git cat-file -e`, which succeeds for dangling objects; the correct test is
`git merge-base --is-ancestor <sha> HEAD`. 1-39 has been corrected.

**The rebase.** 163 commits replayed onto `origin/develop` (4 incoming commits: PR template, 1.10.2-SNAPSHOT
bump, release-workflow auth, XNAT-8801 `xnat-data-builder`). One conflict, in `gradle.properties` — develop
moved to `1.10.2-SNAPSHOT`, this branch sets `1.11.0-SNAPSHOT`; resolved in favour of the branch, which is
the point of the branch. Backup: `backup/jakarta-cutover-c7165817b` (local-only).
- **Structural check passed exactly:** develop is an ancestor; commit count unchanged at 163; and
  `git diff <backup> HEAD --name-only` equals develop's incoming file list **minus `gradle.properties`** —
  the single expected difference, since both backup and HEAD carry `1.11.0-SNAPSHOT`. The rebase introduced
  nothing of its own.
- **Bug-class sweep clean.** Develop's incoming *added lines* (106, hunks only) searched for the recurring
  "develop code written against the pre-cutover stack" class — `javax.`, `Representation.createEmpty`,
  `com.noelios`, springfox, `WebMvcConfigurerAdapter`, `antMatchers`, `HttpMethod.resolve`, `RunData`,
  `authorizeRequests`: **0 hits on all nine**. Develop brought **no Java or Velocity source at all** this
  time, which is why — the class was not exercised rather than proven absent.
- **Verify build green:** `./gradlew compileJava compileTestJava verifyNoOrphanedJavaxEE` — BUILD SUCCESSFUL.

**Re-pointing.** Each cited SHA was matched to its post-rebase equivalent by **`git patch-id --stable`**,
which hashes the diff and ignores the parent, so it identifies the same change after relocation. Result:
- **16 re-pointed**, all matched by patch-id — none needed the subject-line fallback.
- **5 needed no change**: they are commits on `develop`'s permanent history (e.g. `a2559a027`, `ea0f556b9`),
  unaffected by rebasing this branch.
- **1 is permanently dead and was annotated, not faked:** `f13a02162` is a commit on
  `features/turbine7-clean`, a *different* branch since deleted from local and origin. Re-pointing it at a
  cutover commit would assert something false — it was the source that was adapted per-hunk, and the
  resulting commit on this branch (`bece09806`) is cited separately. The line now says the SHA no longer
  resolves and why.
- Verified afterwards: **21 of 22 cited SHAs are reachable from HEAD**, the 1 exception being the annotated
  one; and a SHA-masked diff of the file confirms **only SHA tokens changed** — no prose was disturbed.

**Policy from here: merge `develop` in, do not rebase.** Once the PR is open, a force-push detaches review
comments across a ~780-file diff and forces reviewers to restart; and every further rebase re-orphans these
