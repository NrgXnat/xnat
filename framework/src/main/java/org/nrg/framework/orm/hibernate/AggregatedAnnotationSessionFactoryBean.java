/*
 * framework: org.nrg.framework.orm.hibernate.AggregatedAnnotationSessionFactoryBean
...
public class AggregatedAnnotationSessionFactoryBean extends LocalSessionFactoryBean {
    ...
    public void overridePackagesToScan(final String[] packagesToScan) {
        _packagesToScan.clear();
        setPackagesToScan(packagesToScan);
    }

    @SuppressWarnings("unused")
    public void overrideEntityPackageLists(final List<HibernateEntityPackageList> packageLists) {
        _packagesToScan.clear();
        setEntityPackageLists(packageLists);
    }
    ...
    private Set<String> getXnatEntityPackages(final String[] resourcePaths) {
        ...
        for (final String resourcePath : resourcePaths) {
            log.debug("Searching for entity package definitions using the resource path: {}", resourcePath);
            try {
                for (final Resource resource : BasicXnatResourceLocator.getResources("classpath*:" + resourcePath)) {
                    log.debug("Processing entity packages from the resource: {}", resource.getFilename());
                    try (final InputStream input = resource.getInputStream()) {
                        packages.addAll(IOUtils.readLines(input, "UTF-8"));
                    }
                }
            } catch (IOException e) {
                log.error("An error occurred trying to locate resources on the path: classpath*:{}", resourcePath, e);
            }
        }
        if (_pluginBeanManager != null) {
            for (final String pluginId : _pluginBeanManager.getPluginIds()) {
                ...
                log.debug("Processing entity packages from plugin {}: {}", plugin.getId(), String.join(", ", plugin.getEntityPackages()));
