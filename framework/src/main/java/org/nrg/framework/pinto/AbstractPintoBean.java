/*
 * framework: org.nrg.framework.pinto.AbstractPintoBean
...
public abstract class AbstractPintoBean {
     ...
     * continuing to include specifying help or version parameters.
     *
    ...
    public void displayHelp() {
        if (_parametersByShortOption.isEmpty()) {
        ...
        for (ParameterData parameter : _parametersByShortOption.values()) {
            ...
            getPrintStream().println(parameterText);
        }
    }
     ...
     * @return A map with the list items separated into key-value pairs.
     */
    protected Map<String, String> addKeyValueListToParameter(final List<String> keyValues, @SuppressWarnings("SameParameterValue") final String delimiter) {
    ...
    private void scan() throws PintoException {
        ...
        for (Method method : methods) {
            ...
            if (annotation != null) {
                log.debug("Found command-line parameter annotation {} on {}.{}() method", annotation.value(), getClass().getName(), method.getName());
        ...
        for (String argument : _arguments) {
            ...
            if (isParameter(argument)) {
                ...
                if (_parameters.containsKey(parameter.getShortOption())) {
                    ...
                    _parameters.put(parameter.getShortOption(), new ArrayList<>());
                }
    ...
    private void validateArgCount(final ParameterData parameter, final List<String> arguments) throws PintoException {
        ...
        switch (ArgCount) {
            case StandAlone:
                log.debug("Found parameter {} specified as StandAlone parameter, comes with {} arguments", parameterId, argCount);
                if (argCount > 0) {
                    throw new PintoException(PintoExceptionType.SyntaxFormat, "The parameter " + parameterId + " does not accept any arguments.");
                }
                break;
            case OneArgument:
                log.debug("Found parameter {} specified as OneArgument parameter, comes with {} arguments", parameterId, argCount);
                ...
                break;
            case OneToN:
                log.debug("Found parameter {} specified as OneToN parameter, comes with {} arguments", parameterId, argCount);
                if (argCount == 0) {
            ...
            case SpecificCount:
                ...
                log.debug("Found parameter {} specified as SpecificCount parameter with {} arguments required, comes with {} arguments", parameterId, acceptedArgCount, argCount);
                ...
                break;
            case ZeroToN:
                log.debug("Found parameter {} specified as ZeroToN parameter, comes  with {} arguments", parameterId, argCount);
        }
    }
    ...
    private final Map<String, List<String>>  _parameters                  = new LinkedHashMap<>();
    private final List<String>               _trailing                    = new ArrayList<>();
    private final Map<String, ParameterData> _parametersByShortOption     = new HashMap<>();
    private final Map<String, ParameterData> _parametersByLongOption      = new HashMap<>();
    private final Map<String, ParameterData> _parametersWithDefaultValues = new HashMap<>();
    private final AtomicBoolean              _version                     = new AtomicBoolean(false);
    private final AtomicBoolean              _help                        = new AtomicBoolean(false);
