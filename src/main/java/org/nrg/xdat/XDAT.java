...
public class XDAT implements Initializable, Configurable {
	...
	public static void setNewUserDetails(XDATUserDetails userDetails,
		...
		try {
			...
			item.setProperty("xdat:user_login.ip_address",
					AccessLogger.GetRequestIp(data.getRequest()));
			...
			AccessLogger
					.LogActionAccess(data, "Valid Login:" + user.getLogin());
	...
	public void configure(Configuration configuration) {
		instanceSettingsLocation = configuration
				.getString("instance_settings_directory");
	}
	...
	public static void init(String location, boolean allowDBAccess)
			throws Exception {
		init(location, allowDBAccess, true);
	}

	public static void init(String location, boolean allowDBAccess,
			boolean initLog4j) throws Exception {
		DisplayManager.clean();
		if (initLog4j) {
		...
		if (allowDBAccess) {
			try {
				for (ElementSecurity es : ElementSecurity
						.GetQuarantinedElements()) {
					GenericWrapperElement.GetElement(es.getElementName())
							.setQuarantineSetting(
									es.getBooleanProperty(
											ViewManager.QUARANTINE, false));
				}

				for (ElementSecurity es : ElementSecurity.GetPreLoadElements()) {
					GenericWrapperElement.GetElement(es.getElementName())
							.setPreLoad(
									es.getBooleanProperty("pre_load", false));
				}
			} catch (Exception e) {
				logger.error("", e);
			}
	...
	/**
	 * Returns an instance of the Spring application context service. This
	 * provides a wrapper around the Spring application context to allow
	 * non-Spring components in XNAT to access the context.
	 * 
	 * @return An instance of the {@link ContextService Spring application
	 *         context service}.
	 */
	 ...
	 * {@link NotificationService notification service}.
	 * 
	 * @return An instance of the {@link NotificationService notification
	 *         service}.
	 */
	public static NotificationService getNotificationService() {
		if (_notificationService == null) {
			_notificationService = getContextService().getBean(
	...
	public static PropertiesService getPropertiesService() {
		if (_propertiesService == null) {
			_propertiesService = getContextService().getBean(
	...
	/**
	 * Returns an instance of the currently supported
	 * {@link XftFieldExclusionService exclusion service}.
	 * 
	 * @return An instance of the {@link XftFieldExclusionService exclusion
	 *         service}.
	 */
	public static XftFieldExclusionService getExclusionService() {
		if (_exclusionService == null) {
			_exclusionService = getContextService().getBean(
					org.nrg.xft.services.XftFieldExclusionService.class);
		}
	...
	/**
	 * Returns an instance of the currently supported
	 * {@link MarshallerCacheService XML marshaler cache service}.
	 * 
	 * @return An instance of the {@link MarshallerCacheService XML marshaller
	 *         cache service}.
	 */
	public static MarshallerCacheService getMarshallerCacheService() {
		if (_marshallerCacheService == null) {
			_marshallerCacheService = getContextService().getBean(
					MarshallerCacheService.class);
		}
	...
	/**
	 * Returns the folder containing screen templates. These are installed by
	 * custom datatypes, modules, and other customizations that extend or
	 * override the default application behavior.
	 * 
	...
	public static File getScreenTemplatesSubfolder(String subfolder) {
		...
		for (String folder : subfolders) {
			...
			if (!current.exists()) {
				logger.error("", new NrgRuntimeException(
						"The folder indicated by " + current.getAbsolutePath()
								+ " doesn't exist."));
				return null;
			}
			if (!current.isDirectory()) {
				logger.error("", new NrgRuntimeException(
						"The path indicated by " + current.getAbsolutePath()
								+ " isn't a folder."));
				return null;
			}
		}
	...
	public static ConfigService getConfigService() {
		if (_configurationService == null) {
			_configurationService = getContextService().getBean(
					ConfigService.class);
		}
	...
	/**
	 * This verifies that a notification and subscriber exists for the indicated
	 * site-wide event. If the notification or subscriber does <i>not</i> exist,
	 * one is created using the primary administrative user.
	 * 
	 * @param event
	 *            The site-wide event to be verified.
	 */
	public static void verifyNotificationType(NotificationType event) {
		...
		Category category = getNotificationService().getCategoryService()
				.getCategoryByScopeAndEvent(CategoryScope.Site, event);
		if (category == null) {
			category = getNotificationService().getCategoryService()
					.newEntity();
		...
		if (definitions == null || definitions.size() == 0) {
			definition = getNotificationService().getDefinitionService()
					.newEntity();
		...
		Subscriber subscriber = getNotificationService().getSubscriberService()
				.getSubscriberByName(ADMIN_USERNAME_FOR_SUBSCRIPTION);
		if (subscriber == null) {
			try {
				subscriber = getNotificationService().getSubscriberService()
		...
		List<Subscription> subs = getNotificationService()
		...
		for (Subscription sub : subs) {
			...
			if (sub.getSubscriber().getId() == (subscriber.getId())
					&& sub.getSubscriberType().equals(SubscriberType.User)
					&& ids.contains(channel.getId())) {
				found = true;
				break;
			}
		}
		if (!found) {
			getNotificationService().subscribe(subscriber, SubscriberType.User,
					definition, channel);
		}
	}

	/**
	 * Retrieves the HTML mail channel to be used for default subscription
	 * configuration.
	 * 
	 * @return The HTML mail channel definition.
	 */
	private static Channel getHtmlMailChannel() {
		Channel channel = getNotificationService().getChannelService()
				.getChannel("htmlMail");
		if (channel == null) {
