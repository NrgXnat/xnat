/*
 * xnat-data-models: org.nrg.xdat.om.base.BaseXnatImagesessiondata
...
public abstract class BaseXnatImagesessiondata extends AutoXnatImagesessiondata {
    ...
    public ArrayList<XnatImagescandata> getScansByType(String type) {
        ArrayList<XnatImagescandata> _return = new ArrayList<>();
    ...
    public ArrayList<XnatImagescandata> getScansByTypeList(List<String> typeList) {
        ArrayList<XnatImagescandata> _return = new ArrayList<>();
        HashSet<XnatImagescandata> tempSet = new HashSet<>();
    ...
    /**
     * @param typeCsv   Comma-separated string of scan types (literal or regex) to retrieve from the session
     * @return ArrayList of org.nrg.xdat.om.XnatImagescandata
     */
    public ArrayList<XnatImagescandata> getScansByTypeCsv(String typeCsv) {
        return StringUtils.isNotBlank(typeCsv)
               ? getScansByTypeList(Arrays.stream(typeCsv.split(",")).map(String::trim).collect(Collectors.toList()))
               : new ArrayList<>();
    }
    ...
    /**
     * @param label    The label of the assessor to retrieve.
     * @return The requested image assessor data object.
     */
    public XnatImageassessordata getAssessorByLabel(String label) {
        for(XnatImageassessordataI assessor : getAssessors()){
            if (assessor.getLabel().equalsIgnoreCase(label)) {
                return (XnatImageassessordata) assessor;
            }
        }
        return null;
    }
	...
	private List<XnatQcmanualassessordata> sortedqc=null;
	public List<XnatQcmanualassessordata> getSortedQC(){
		if(sortedqc==null){
			sortedqc=this.getMinimalLoadAssessors(XnatQcmanualassessordata.SCHEMA_ELEMENT_NAME);

			Collections.sort(sortedqc,new Comparator<XnatQcmanualassessordata>(){
				@Override
				public int compare(XnatQcmanualassessordata arg0,
						XnatQcmanualassessordata arg1) {
					if(arg1.getInsertDate()!=null){
						return arg1.getInsertDate().compareTo(arg0.getInsertDate());
					}else{
						if(arg0.getInsertDate()!=null){
							return -1;
						}else{
							return 0;
						}
	...
	public XnatQcscandata getManualQCforScan(String id){
		for(XnatQcmanualassessordata qc: getSortedQC()){
			try {
				return (XnatQcscandata)qc.getScanById(id);
			} catch (Exception e) {
				//ignore
			}
		}
	...
	public List<XnatQcscandata> getAllManualQCforScan(String id){
		List<XnatQcscandata> scans=new ArrayList<>();
		for(XnatQcmanualassessordata qc: getSortedQC()){
			try {
				scans.add((XnatQcscandata)qc.getScanById(id));
			} catch (Exception e) {
				//ignore
			}
		}
		...
		for (final XnatImagescandataI scan : this.getScans_scan()){
		    ...
		    if (null != otherScan){
                ...
                if (null != this.getProject())
                    ((XnatImagescandata)scan).setProject(this.getProject());
		    }
		    scancounter++;
		}
    }
...
//    }
    ...
    public Map<String,String> getCustomScanFields(String project){
        return new LinkedHashMap<>(CUSTOM_SCAN_FIELDS);
    }

    public void moveToProject(final XnatProjectdata newProject, final String label, final UserI user,
                              final EventMetaI c, final List<String> assessorsToMove) throws Exception {
        final String destinationProject = newProject.getId();
        if (StringUtils.equals(getProject(), destinationProject)) {
            return;
        }
        ...
        final File                     rootBackup       = MoverMaker.createPrimaryBackupDirectory("move", getProject(), getId());
        final String                   existingRootPath = getProjectData().getRootArchivePath();
        final String                   currentLabel     = StringUtils.defaultIfBlank(getLabel(), getId());
        final String                   newLabel         = StringUtils.defaultIfBlank(label, currentLabel); //FIXME: Is this correct?
        ...
        final Map<String, File> fs         = new HashMap<>();
        final File              sessionDir = getSessionDir();
        if (sessionDir != null) {
            fs.put("src", sessionDir);
        }

        final CopyOp scanOp = new CopyOp(new OperationI<Map<String, File>>() {
            public void run(final Map<String, File> fs) throws Exception {

                if(XDAT.getBoolSiteConfigurationProperty("rerunProjectAnonOnRename", false)) {
                    new ProjectAnonymizer(base, newProject.getId(), base.getArchivePath(existingRootPath), true).call();
                }

                for (final XnatImagescandataI scan : getScans_scan()) {
                    for (final XnatAbstractresourceI abstRes : scan.getFile()) {
                        final MoverMaker.Mover mover = MoverMaker.moveResource(abstRes, currentLabel, base,
                                newSessionDir, existingRootPath, destinationProject, user, c);
                ...
                for (final XnatReconstructedimagedataI recon : base.getReconstructions_reconstructedimage()) {
                    for (final XnatAbstractresourceI abstRes : recon.getOut_file()) {
                        final MoverMaker.Mover mover = MoverMaker.moveResource(abstRes, currentLabel, base,
                                newSessionDir, existingRootPath, destinationProject, user, c);
                        mover.setResource((XnatAbstractresource) abstRes);
                        mover.call();
                ...
                for (final XnatImageassessordataI assessor : base.getAssessors_assessor()) {
                    // assessors need to be updated
                    if (assessorsToMove != null && assessorsToMove.contains(assessor.getId())) {
                        final EventDetails event = EventUtils.newEventInstance(EventUtils.CATEGORY.DATA,
                                EventUtils.TYPE.WEB_SERVICE, "Moved assessor to new project.");
                        XnatImageassessordata.ChangePrimaryProject(user, (XnatImageassessordata) assessor,
                                newProject, null, event, null);
                    }

                    final Set<XnatAbstractresourceI> resources = new HashSet<>(assessor.getOut_file());
                    resources.addAll(assessor.getResources_resource());
                    for (final XnatAbstractresourceI resource : resources) {
                        final MoverMaker.Mover mover = MoverMaker.moveResource(resource, currentLabel, base,
                                newSessionDir, existingRootPath, destinationProject, user, c);
        ...
        try {
            new TransactionRunner<Void>().runTransaction(scanOp);
    ...
    @Override
    public String delete(final BaseXnatProjectdata project, final UserI user, final boolean removeFiles, final EventMetaI c) {
        final BaseXnatImagesessiondata experiment = getItem().getUser() != null ? new XnatImagesessiondata(this.getCurrentDBVersion(true)) : this;
        final String               message    = experiment.canDelete(project, user);
        if (StringUtils.isNotBlank(message)) {
            logger.error(message);
            return message;
        }

        if (!experiment.getProject().equals(project.getId())) {
            return deleteShared(user, project, experiment, c);
        }
        ...
        try {
            ...
            SaveItemHelper.authorizedDelete(experiment.getItem().getCurrentDBVersion(), user, c);
    ...
    public void deleteFiles(final UserI user, final EventMetaI ci) throws Exception {
        final List<XnatAbstractresourceI> resources = new ArrayList<>(getResources_resource());
        for (final XnatImagescandataI scan : getScans_scan()) {
            resources.addAll(scan.getFile());
        }
        for (final XnatReconstructedimagedataI scan : getReconstructions_reconstructedimage()) {
            resources.addAll(scan.getOut_file());
        }
        for (final XnatImageassessordataI scan : getAssessors_assessor()) {
            resources.addAll(scan.getResources_resource());
            resources.addAll(scan.getOut_file());
        }
	...
	public void preSave() throws Exception{
		...
		for(final XnatImagescandataI scan:this.getScans_scan()){
			...
			scan.setProject(this.getProject());
		}
	...
	/**
	 * If the pipeline is registered for this experiment's primary project or shared projects, then the full pipeline path will be returned.
	 * @param user
	 * @param pipelineStep
	 * @return
	 */
	public ArcPipelinedataI canRunPipeline(UserI user,String pipelineStep){
		ArcPipelinedataI pipeline=null;
		try {
		...
		if(pipeline==null){
			for(XnatExperimentdataShareI share: this.getSharing_share()){
				try {
					//check if the user can modify data in this project
					if(Permissions.canEdit(user, this.getItem().getXSIType()+"/project", this.getProject())){
    ...
    private String deleteShared(final UserI user, final BaseXnatProjectdata project, final BaseXnatImagesessiondata experiment, final EventMetaI c) {
        try {
            SecurityValues values = new SecurityValues();
            values.put(getXSIType() + "/project", project.getId());

            if (!Permissions.canDelete(user, experiment) && !Permissions.canDelete(user, this.getSchemaElement(), values)) {
                return "User cannot delete experiments for project " + project.getId();
            }

            //unshare children before unsharing parent
            for (final XnatImagescandataI scani : experiment.getScans_scan()) {
                final XnatImagescandata scan = (XnatImagescandata) scani;
                if (scan.getProject().equals(project.getId())) {
                    continue;
                }
                int index = 0;
                int match = -1;
                for (XnatImagescandataShareI pp : scan.getSharing_share()) {
                    if (pp.getProject().equals(project.getId())) {
                        SaveItemHelper.authorizedRemoveChild(scan.getItem(),
                                                             "xnat:imageScanData/sharing/share", ((XnatImagescandataShare) pp).getItem(),
                                                             user, c);
                        match = index;
                        break;
                    }
                ...
                if (match != -1) {
                    scan.removeSharing_share(match);
                }
            }

            final List<XnatImageassessordata> assessors = (experiment).getAssessors_assessor();
            for (final XnatImageassessordata assessor : assessors) {
                if (assessor.getProject().equals(project.getId())) {
                    return "This operation would delete an experiment (rather than un-share).  Please move experiment (" + experiment.getId() + ") to another project or manually delete.";
                }
                final String delete = assessor.delete(project, user, false, c);
                if (StringUtils.isNotBlank(delete)) {
                    return delete;
            ...
            for (XnatExperimentdataShareI pp : experiment.getSharing_share()) {
                if (pp.getProject().equals(project.getId())) {
                    SaveItemHelper.authorizedRemoveChild(experiment.getItem(), "xnat:experimentData/sharing/share", ((XnatExperimentdataShare) pp).getItem(), user, c);
                    match = index;
                    break;
                }
                index++;
